...
function setupDashboardChart(containerElementId) {
    ...
    var s1 = [200, 300, 400, 500];
    var s2 = [190, 290, 390, 490];    //
    //var s3 = [250, 350];    //, 450, 550
    //var s4 = [2000, 1600];  //, 1400, 1100
    var ticks = ['华北片区','华中片区','华南片区','西北片区']
...
function setupDialogBoxByWidth(containerElement, associatedButton, wd) {
    $.fx.speeds._default = 1000;
    $("#" + containerElement).dialog({
        autoOpen: false,
        show: "blind",
...
function setupDialogBox(containerElement) {
    $.fx.speeds._default = 1000;
    $("#" + containerElement).dialog({
        autoOpen: false,
        show: "blind",
...
/*update: add 2016-6-10 23:39:38 时间函数*/
function getNowDate() {
    var now = new Date();
...
function getPreWeekDate(){
	 var now = new Date(); 
	 var nextDate = new Date(now.getTime() - 7*24*60*60*1000);  // 前一周时间
	 return getFormatDate(nextDate);
}
...
function setupDialogBoxByWidth(containerElement, wd, ht) {
    $.fx.speeds._default = 1000;
    $("#" + containerElement).dialog({
        autoOpen: false,
        show: "blind",
